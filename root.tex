...
\author{E Li$^{1}$, Chengyang Li$^{1}$,Shuaijun Wang$^{1,2}$, Xiangbin Wu$^{3}$, and Qi Hao$^{1,*}$% <-this % stops a space
...
\begin{figure}[ht]
 ...
 %\resizebox{\linewidth}{!}{
 \includegraphics[width=0.9\linewidth]{./figures/main_ui}\\ %\vspace{-0.3cm}
...
We follow the listed philosophies below to create our tool:
...
We provide a focus mode, when activated for a selected object, the object is centered and zoomed in with backgrounds removed, such that a user can inspect all annotation details. Without this feature a use has to pan,  zoom and rotate the main view, which we think is a lot operations and hard work.

\begin{figure}[thpb]
	\centering	
	\includegraphics[width=0.9\linewidth]{./figures/focus-mode}\\ 
	\caption{In focus mode, the target object is automatically centered and zoomed in with most background removed, all editing features are still applicable.}
...
Given source and target objects (denoted by $s,t$ respectively)and their bounding boxes ($b_s,b_t$), a registration algorithm finds the transform matrix $T$ to match $s$ with $t$ , satisfying
$$T(s)=t$$
where we represent $s$ and $t$ in the corresponding bounding box coordinates $B_s$ and $B_t$.
obviously $b_s,b_t$ should have the same relation
$$T(b_s)=b_t$$
